...
<script>
  var demo = new Vue({
    ...
    ,
    methods: {
         //ตรวจสอบเวลาว่าซ้ำกับเวลาที่เคยมีการจัดหรือไม่
      checkTime(){
        var self = this;
        ...
        if(this.date_time_start_mili!=this.start_time_check&&this.date_time_end_mili!=this.end_time_check){
          for(var i of roomuseDate){ // ตรวจสอบเวลาสอบที่เลือกกับข้อมูลในฐานข้อมูลว่ามีการทับศ้อนกันหรือไม่
    ...
    validate_size(){
      ...
      if(lastSize == 0){//ตรวจสอบขนาดของข้อมูลในทุกรายวิชารวมกันว่า == 0 หรือไม่
        return true;
      }
      else{
  ...
  processData(){
      ...
      this.allSubCode=[];
      for(let data of temp){// ทำการวน loop เพื่อทำการเก็บข้อมูลที่ถูกต้องลงตัวแปรก่อนประมวลผล
  ...
  //กดเพื่อทำการเลือกวิชาในการจัดสอบ
  selectSubject(id){
      let self = this;
      if(this.subject == id){
        return 0;
        ...
        this.subject = id;//กำหนดรหัสวิชาที่ถูกเลือก
      for(let i = 0 ;i<self.fileSize.length;i++){
        if(self.fileSize[i].id ==id){
          this.itemSize = self.fileSize[i].size;
          if(!this.person[this.subject]){// โหลดข้อมูลรายชื่อให้ครงกับวิชาที่เลือก
  ...
  checkAll_validate(c){
    ...
    checkOne(id){
      if(!this.subject){//ตรวจสอบว่าเลือกรายวิชาหรือยัง
        return 0;
      }
      if(this.checkOne_validate(id)== false){// ตรวจสอบว่าเคยมีการเลือกที่นั่งนี้หรือไม่
        return 0;
      }
      let c=id.substr(0,1);
      if(this.col_name[c]!=c){// ตรวจสอบว่าเคยมีการเลือกที่นั่งนี้หรือไม่
        this.col_name[c]=c;
        this.col_check = 0;
        this.tempData = [];
      }
      else if(this.col_name[c]==c){//หากเป็นรายวิชาเดิมให้ทำการเตรียมข้อมูลที่ตรงกับคอลัมน์ที่เลือก
        this.tempData = [];
      ...
      if(this.checkSubject==""){// ทำงานเมื่อมีการเลือกรายวิชาที่ไม่เคยมีการเลือกมาก่อน
        this.allSubject_id.find(element => element.id == this.subject).size -=1;
        ...
        if(!this.search_seat(id)){//ทำงานเมือมีการกดที่นั่งในตำแหน่งที่ถูกเลือกไปก่อนแล้ว
          alert("ที่นั่งถูกใช้แล้ว");
          return 0;
        }
        this.personValue = this.person[this.subject].shift();//ลบรายชื่อผู้มีสิทธิ์สอบในตำแหน่งบนสุดของ Array 
        this.tempData.push({//ทำการเก็บข้อมูลรายชื่อ รหัสนิสิต ชั้นปั ห้องสอบ ชื่อวิชาสอบ ลงตัวแปร
      ...
      else if(this.checkSubject!=this.subject){//ทำงานเมื่อมีการเลือกรายวิชาที่เคยมีการเลือกแล้ว
        if(!this.search_seat(id)){//ตรวจสอบตำแหน่งที่นั่งจาก หมายเลขที่นั่ง
          if(this.arr3d[c].find(x=> x.seat == id && x.subject == this.subject)){//ทำงานเมื่อมีกดลงที่นั่งที่เคยเลือกเพื่อยกเลิกที่นั่ง
            $('#'+id).css({// เปลี่ยน css ของตำแหน่งที่นั่งให้มีสถานะเป็นว่าง
              "background-color": "transparent","color":"black"}
                         );
            this.removeData_seat(c,id);//ทำการนำข้อมูลออกจากตัวแปรที่จัดที่นั่ง
            return 0;
          }
          else{ //ทำงานเมือมีการกดที่นั่งในตำแหน่งที่ถูกเลือกไปก่อนแล้ว
            alert("ที่นั่งถูกใช้แล้ว");
            return 0;
          }
        }
        if((this.allSubject_id.find(element => element.id == this.subject).size != 0)){ //ทำงานเมื่อมีกดเลือกตำแหน่งที่นั่งที่ว่าง
          this.allSubject_id.find(element => element.id == this.subject).size -=1;//ลดจำนวนรายชื่อผู้มีสิทธิ์สอบลง 1 
          this.personValue = this.person[this.subject].shift();//ลบรายชื่อผู้มีสิทธิ์สอบในตำแหน่งบนสุดของ Array 
          this.checkSubject=this.subject;
          $("input[id='"+id+"']").prop("checked", !this.clicked2[id]);
          $('#'+id).css({
            "background-color": this.randomColor(this.subject),"color":"white"}// เปลี่ยน css ของตำแหน่งที่นั่งให้มีสถานะเป็นถูกเลือก โดยการสุ่มสีตามการ์ดรายวิชา
                       );
          this.tempData.push({//ทำการเก็บข้อมูลรายชื่อ รหัสนิสิต ชั้นปั ห้องสอบ ชื่อวิชาสอบ ลงตัวแปร
            seat:id,subject:this.subject,name:this.personValue.ชื่อ,id:this.personValue.รหัสนิสิต,year:this.personValue.ชั้นปี,group:this.personValue.กลุ่ม,room:this.selectFloor}
                            );
      ...
      else if(this.checkSubject==this.subject){// ทำงานเมื่อเลือกรายวิชาที่เคยจัดที่นั่งแล้ว
        if(!this.search_seat(id)){//ตรวจสอบตำแหน่งที่นั่งจาก หมายเลขที่นั่ง
          if(this.arr3d[c].find(x=> x.seat == id && x.subject == this.subject)){//ทำงานเมื่อมีกดลงที่นั่งที่เคยเลือกเพื่อยกเลิกที่นั่ง
            $('#'+id).css({
              "background-color": "transparent","color":"black"}// เปลี่ยน css ของตำแหน่งที่นั่งให้มีสถานะเป็นว่าง
                         );
            this.removeData_seat(c,id);
            return 0;
          }
          else if(this.seat_check.find(e => e === id)){//ทำงานเมือมีการกดที่นั่งในตำแหน่งที่ถูกเลือกไปก่อนแล้ว
            alert("ที่นั่งถูกใช้แล้ว");
            return 0;
          }
        }
  ...
  ,
    ...
    checkAll(c){
      ...
      if(this.checkAll_validate(c)==false){//ตรวจสอบว่าเคยมีการเลือกที่นั่งคอลัมน์นี้หรือไม่
        return 0;
      }
      ...
      let a = 1;
      let self = this;
      if(this.col_name[c]!=c){//ตรวจสอบว่ามีการกดเปลี่ยนรายวิชาหรือไม่
        this.col_name[c]=c;
        this.col_check = 0;
        this.tempData = [];
      }
      ...
      if(this.checkSubject==""){// ทำงานเมื่อมีการเลือกรายวิชาที่ไม่เคยมีการเลือกมาก่อน
        ...
        for(i=1;i<=this.rowAll;i++){
          if(i>this.rowAll){ //ทำการหยุดการทำงานของ loop เมื่อ i > จำนวนแถว
              throw("error");
          }
          if((this.allSubject_id.find(element => element.id == this.subject).size != 0)){
            if(this.search_seat(c+i)){//ตรวจสอบตำแหน่งที่นั่งจาก หมายเลขที่นั่ง
              this.personValue = this.person[this.subject].shift();//ลบรายชื่อผู้มีสิทธิ์สอบในตำแหน่งบนสุดของ Array
              this.allSubject_id.find(element => element.id == this.subject).size -=1;//ลดจำนวนรายชื่อผู้มีสิทธิ์สอบลง 1 
            ...
            else{//ทำงานเมือมีการกดคอลัมในตำแหน่งที่ถูกจัดจนเต็มแล้ว
              alert("แถวเต็มแล้ว");
              return 0;
            }
          }
      ...
      else if(this.checkSubject!=this.subject){//ทำงานเมื่อมีการเลือกรายวิชาที่ไม่เคยมีการเลือก
        this.checkSubject=this.subject 
        let status = true;
        if(this.arr3d[c] != null){// ทำงานเมื่อมีการเลือกคอลัมน์ที่ถูกไปแล้วเพื่อนำข้อมูลออก
          for(i=1;i<=this.rowAll;i++){
            if(this.arr3d[c].find(x=> x.seat == c+i && x.subject == this.subject)){//ทำงานเมื่อมีกดลงที่นั่งที่เคยเลือกเพื่อยกเลิกที่นั่ง
              $('#'+c+i).css({
                "background-color": "transparent","color":"black"}// เปลี่ยน css ของตำแหน่งที่นั่งให้มีสถานะเป็นว่าง
                            );
        ...
        if(status == true && this.seat_check.find(e => e != c+i)){//ทำงานเมือมีคอลัมถูกเลือกแล้วแต่ยังคงมีที่นั่งเหลืออยู่
          for(i=1;i<=this.rowAll;i++){
            if(i>this.rowAll){ //ทำการหยุดการทำงานของ loop เมื่อ i > จำนวนแถว
              throw("error");
            }
        ...
        if(this.arr3d[c] != null){// ทำงานเมื่อมีการเลือกคอลัมน์ที่ถูกไปแล้วเพื่อนำข้อมูลออก
          for(i=1;i<=this.rowAll;i++){
            if(this.arr3d[c].find(x=> x.seat == c+i && x.subject == this.subject)){//ทำงานเมื่อมีกดลงที่นั่งที่เคยเลือกเพื่อยกเลิกที่นั่ง
              ...
              this.removeData_seat(c,c+i);//ทำการนำข้อมูลออกจากตัวแปรที่จัดที่นั่ง
              status = false;
            }
          }
        }
        if(status == true ){//ทำงานเมือมีคอลัมถูกเลือกแล้ว
          for(i=1;i<=this.rowAll;i++){
            if(i>this.rowAll){ //ทำการหยุดการทำงานของ loop เมื่อ i > จำนวนแถว
              throw("error");
            }
